...
def apply_count_farm_external(hotfix: Path, source: Path) -> tuple[bool, str]:
    ...
    out = (proc.stdout or "") + (proc.stderr or "")
    if proc.returncode != 0:
        return False, out.strip() or "计数挂机·DLL版补丁失败"
    if "[SKIP]" in out and "计数挂机" in out:
        return True, "计数挂机·DLL版：已是补丁状态（跳过）"
...
def apply_auto_point_external(hotfix: Path, source: Path) -> tuple[bool, str]:
    """一键加点·DLL版：编译部署 SeqChapterAutoPoint.dll.bytes（面板脚本页按钮）。"""
    proc = run_patcher_capture(
        [
            "auto-point-external-patch",
            "--hotfix",
            str(source),
            "--output",
            str(hotfix),
    ...
    out = (proc.stdout or "") + (proc.stderr or "")
    if proc.returncode != 0:
        return False, out.strip() or "一键加点·DLL版补丁失败"
    if "[SKIP]" in out and "一键加点" in out:
        return True, "一键加点·DLL版：已是补丁状态（跳过）"
...
def _apply_gameplay_patches(
    ...
    *,
    ...
    # 计数挂机：仅部署 DLL，由面板战斗页互斥切换
    if count_farm:
        _emit_combo(messages, on_log, "正在部署：计数挂机…")
        ok, msg = apply_count_farm_external(hotfix, work)
        if not ok:
            raise RuntimeError(msg)
        _emit_combo(messages, on_log, msg)
        work = hotfix
    ...
    # 一键加点：仅部署 DLL，由面板脚本页按钮触发
    if auto_point:
        _emit_combo(messages, on_log, "正在部署：一键加点…")
        ok, msg = apply_auto_point_external(hotfix, work)
        if not ok:
            raise RuntimeError(msg)
        _emit_combo(messages, on_log, msg)
        work = hotfix
...
def apply_combo(
    *,
    ...
    # 护航面板龙族循环按钮（flag 控制显示）
    dragon_flag = sync_dragon_loop_flag(hotfix.parent, dragon_loop_ui)
    _emit_combo(
        messages,
        on_log,
        "护航面板："
        + ("已开启龙族循环按钮" if dragon_flag else "不显示龙族循环按钮"),
    )
