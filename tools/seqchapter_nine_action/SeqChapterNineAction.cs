...
/// 百科 OnWikiClick：日课流水线（主线程 Timer，不挂钩 NetManager，避免闪退）。
/// </summary>
public static class SeqChapterNineAction
{
    ...
    public static void Bootstrap()
    {
        ...
        try
        {
            EnsureStatusPath();
            CacheHotfixFields();
            WriteStatus("mounted", "nine_daily_timer");
        }
        catch (Exception ex)
        {
            try
    ...
    /// <summary>百科：主线程启动日课。true=已开始；false=进行中。</summary>
    ...
    private static void StartDailyTimer()
    {
        ...
        {
            FinishDaily("日课：Timer 失败 " + ex.GetType().Name);
        }
    }
    ...
    private static void DailyTick()
    {
        ...
        catch (Exception ex)
        {
            ...
            FinishDaily("日课异常：" + ex.GetType().Name);
        }
    }
