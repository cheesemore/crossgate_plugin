...
/// <summary>
/// 抓宠卖银币 DLL（独立于稳定版 SeqChapterAutoCatch）。
/// 部署为 hotfixdata/SeqChapterAutoCatchSell.dll.bytes
/// 战斗内抓宠逻辑与普通抓宠同类；退战时对背包 1 级宠：
///   · 名字已以 # 开头 → 不改名、不回收；
///   · 否则等 ResetBaseInfo：掉档≥Y 且无 @（单项随机&lt;6）→ 回收卖银币；
///   · 其余 → 改名 #档 / #满 / 带 @N；满仓仍走存仓。
/// Y 默认 6，可读 %USERPROFILE%\.seqchapter_helper\catch_sell.json 的 recycle_min_grade。
/// </summary>
public static class SeqChapterAutoCatchSell
{

    public const string AssetPath = "hotfixdata/SeqChapterAutoCatchSell.dll.bytes";
    ...
    /// <summary>掉档 ≥ 此值且无 @ 则回收；默认 6，可被 catch_sell.json 覆盖。</summary>
    private const int DefaultRecycleMinGrade = 6;
    ...
    /// <summary>开启抓宠后标题「★卖银中★遇到1级N只」中的遇一级计数；关闭时清零。</summary>
    ...
    /// <summary>本场首次发现可抓一级时 +1，并刷新标题 ★卖银中★遇到1级N只。</summary>
    ...
    private static void RefreshWindowTitle()
    {
        try
        {
            ...
            if (IsPipelineActive())
            {
                title = title + " ★卖银中★遇到1级" + _levelOneMeetCount + "只";
            }
    ...
    private static void RunExitPipeline(string uid)
    {
        ...
        // 卖银版：改名与回收同一步（停挂机之后、存仓之前）
        TryRenameOrRecycleBagPets(uid);
    ...
    /// <summary>
    /// 1 级宠同一步处理：已 # 开头跳过；掉档≥Y 且无 @ → 回收；其余改名。
    /// </summary>
    private static void TryRenameOrRecycleBagPets(string uid)
    {
        ...
        try
        {
            ...
            var sendRecycle = FindSendPetRecycle(petMgr);
            if (sendChange == null && sendRecycle == null)
            {
                return;
            }
            ...
            var recycleMin = GetRecycleMinGrade();
            ...
            for (var i = 0; i < pets.Count && i < 5; i++)
            {
                ...
                if (ReadIntMember(data, "Level") != StorePetLevel)
                {
                    continue;
                }
                ...
                var hasAt = maxRand >= MinRandomSuffix;
                var index = Convert.ToInt32(GetMember(data, "Index") ?? i);
                var mgrLocal = petMgr;

                // 掉档≥Y 且不会带 @ → 回收卖银；精品（有 @ 或掉档&lt;Y）→ 改名
                if (!perfect && grade >= recycleMin && !hasAt && sendRecycle != null)
                {
                    var indexLocal = index;
                    var sendLocal = sendRecycle;
                    RunProtocolStep(() =>
                    {
                        // SendPetRecycle("回收宠物", uid, petIndex, tab:1, num:0, id:0)
                        sendLocal.Invoke(
                            mgrLocal,
                            new object[] { "回收宠物", uid, indexLocal, 1, 0, 0 });
                    });
                    continue;
                }
                ...
                if (string.IsNullOrEmpty(newName)
                    || string.Equals(display, newName, StringComparison.Ordinal))
                {
                    continue;
                }

                var nameLocal = newName;
                var indexRename = index;
                var sendRename = sendChange;
                RunProtocolStep(() =>
                {
                    sendRename.Invoke(mgrLocal, new object[] { uid, indexRename, nameLocal });
                });
            }
        }
        catch
    ...
    /// <summary>轮询至多 ~2.5s：等未以 # 开头的 1 级宠 ResetBaseInfo 到位。</summary>
            ...
            try
            {
                ...
                if (pets != null)
                {
                    ...
                    for (var i = 0; i < pets.Count && i < 5; i++)
                    {
                        ...
                        var display = GetDisplayPetName(data);
                        if (!MightNeedRenameOrRecycle(display))
                        {
                            continue;
                        }
    ...
    /// <summary>仅未以 # 开头的名字需要等数据后改名或回收。</summary>
    private static bool MightNeedRenameOrRecycle(string display)
    {
        ...
        return !display.StartsWith("#", StringComparison.Ordinal);
    }

    private static int GetRecycleMinGrade()
    {
        try
        {
            foreach (var path in EnumerateCatchSellConfigPaths())
            {
                ...
                if (TryParseRecycleMinGrade(json, out var y) && y >= 0)
                {
                    return y;
                }
            }
        ...
        return DefaultRecycleMinGrade;
    }

    private static string[] EnumerateCatchSellConfigPaths()
    {
        ...
        list.Add(Path.Combine("cg37_Data", "assets", "hotfixdata", "catch_sell.json"));
        return list.ToArray();
    }

    private static bool TryParseRecycleMinGrade(string json, out int value)
    {
        value = DefaultRecycleMinGrade;
        if (string.IsNullOrEmpty(json))
        {
            return false;
        }

        // 轻量解析，避免依赖 JSON 库：找 "recycle_min_grade": <int>
    ...
    private static MethodInfo FindSendPetRecycle(object petMgr)
    {
        if (petMgr == null)
        {
            return null;
        ...
        foreach (var m in petMgr.GetType().GetMethods(
                     BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
        {
            if (m.Name != "SendPetRecycle")
            {
                continue;
            }

            var ps = m.GetParameters();
            // (string type, string uid, int petIndex, int tab, int num, int id)
            if (ps.Length == 6
