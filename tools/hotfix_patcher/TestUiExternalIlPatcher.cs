...
internal static class TestUiExternalIlPatcher
{
    ...
    private static string ResolveSourceDir(string hotfixPath)
    {
        ...
        // 优先：从 hotfixPath 所在目录向上探测游戏根目录（含 cg37_Data 的目录）下的 tools/seqchapter_test_ui。
        ...
        for (var dir = hotfixDir; ; dir = Path.GetDirectoryName(dir)!)
        {
            ...
            probes.Add(Path.Combine(dir, "tools", "seqchapter_test_ui"));
        ...
        probes.Add(Path.GetFullPath(Path.Combine(exeDir, "seqchapter_test_ui")));
        probes.Add(Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "seqchapter_test_ui")));
        probes.Add(Path.GetFullPath(Path.Combine(exeDir, "..", "tools", "seqchapter_test_ui")));
        probes.Add(Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "tools", "seqchapter_test_ui")));
        probes.Add(Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "seqchapter_test_ui")));
        ...
        throw new DirectoryNotFoundException(
            "找不到 tools/seqchapter_test_ui 目录（请把 seqchapter_test_ui 放在 HotfixPatcher.exe 同级，或游戏根目录 tools/ 下）");
    }
