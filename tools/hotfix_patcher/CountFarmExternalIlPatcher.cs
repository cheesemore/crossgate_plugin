...
{
    public const string AssetFileName = "SeqChapterCountFarm.dll.bytes";
    ...
    public static int Run(string[] args)
    {
        ...
        {
            Console.WriteLine(
                "用法: HotfixPatcher count-farm-external-patch --hotfix <orig> --output <out>\n" +
                "      HotfixPatcher count-farm-external-patch --hotfix <file> --detect\n" +
                "      HotfixPatcher count-farm-external-patch --hotfix <orig> --output <out> --restore");
            return 1;
        }
        ...
        {
            Apply(source, output);
            Console.WriteLine("[OK] 计数挂机·DLL版补丁完成: " + output);
            return 0;
        }
        catch (Exception ex)
        {
    ...
    {
        var dllPath = BuildCountFarmDll(sourcePath);
        ...
        Console.WriteLine("[COUNT-FARM] 已部署 " + assetOut);
        Console.WriteLine("[COUNT-FARM] 助手面板战斗页选择「计数挂机」时加载（不改 hotfix IL）");
    }
    ...
    private static string BuildCountFarmDll(string hotfixPath)
    {
        var srcDir = ResolveSourceDir(hotfixPath);
        var csPath = Path.Combine(srcDir, "SeqChapterCountFarm.cs");
        if (!File.Exists(csPath))
        {
            throw new FileNotFoundException("找不到 SeqChapterCountFarm.cs", csPath);
        }

        var hotfixDataDir = Path.GetDirectoryName(hotfixPath)!;
        var outDir = Path.Combine(Path.GetTempPath(), "seqchapter_countfarm_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(outDir);
        var dllPath = Path.Combine(outDir, "SeqChapterCountFarm.dll");
        ...
        if (refs.Count == 0)
        {
            throw new InvalidOperationException("未找到 hotfixdata 内 mscorlib/system，无法编译计数挂机 DLL");
        }
        ...
        if (!result.Success)
        {
            ...
            throw new InvalidOperationException("Roslyn 编译 SeqChapterCountFarm 失败:\n" + errors);
        }

        File.WriteAllBytes(dllPath, ms.ToArray());
        Console.WriteLine($"[COUNT-FARM] 已编译计数挂机 DLL（{refs.Count} 个引用）");
        return dllPath;
    }
