...
状态：**UI 路径已打通；产品「战斗模式切换」尚未开工**（2026-08-01）

### 实机结论修订

| 时间 | 结论 |
|------|------|
| 2026-07-29 | Plugin Host 一期 UGUI 反射失败 → 曾整案搁置 |
| 2026-08-01 | `SeqChapterTestUi`：**Update + UGUI** 可行（HybridCLR **不派发 OnGUI/IMGUI**）。见 `tools/seqchapter_test_ui/README.md` |

### 产品意向（理解备忘，未实现）

百科 → **总面板**（多子切页）：

1. **概况**：队长地图号/坐标/是否战斗；队员每日魔石；队伍血魔池（优先反射读，不为此新打 IL）。
2. **战斗模式**：圆点单选互斥——常规（全关）/ 九动 / 抓宠 / 烧卡 / 抓宠（不带宠）…；傻瓜**九动版**才可选九动。
3. **简单脚本**：如做日常等。

体积：UI+概况读数走外置 DLL；顶 hotfix 的是战斗模式自带的既有钩子，不是概况展示。

以下原文为 07-29 历史设计存档，细节勿直接当主线照做。
